...
作为一个 AI 产品实习生，我希望输入一个行业研究任务后，系统可以自动拆解任务、选择工具、记录执行过程、评估结果质量，并在证据不足时自动补充搜索，这样我可以更快得到一份可解释、可复盘的行业研究结论。
...
面向垂直行业研究场景，设计并实现轻量级 Agent Harness Console，支持研究任务拆解、工具路由、执行链路追踪、质量评分、自动修复和用户反馈导出，模拟大模型从“直接回答”升级为“可调度、可观测、可迭代”的行业研究 Agent。
